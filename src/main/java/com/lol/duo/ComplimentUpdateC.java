...
@WebServlet("/ComplimentUpdateC")
public class ComplimentUpdateC extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		AccountDAO.loginCheck(request);
		ComplimentDAO.getCdao().getCompliment(request);
		ComplimentDAO.getCdao().getAllCompliment(request);
		ComplimentDAO.getCdao().paging(1, request);
		request.setAttribute("contentPage", "compliment_update.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AccountDAO.loginCheck(request);
		ComplimentDAO.getCdao().updateCompliment(request, response);
		ComplimentDAO.getCdao().getAllCompliment(request);
		ComplimentDAO.getCdao().paging(1, request);
