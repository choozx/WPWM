...
@WebServlet("/DuoDelController")
public class DuoDelController extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("contentPage", "duo_delete.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		AccountDAO.loginCheck(request);
		DuoDAO.getDdao().getDuo(request);
		DuoDAO.getDdao().deleteDuo(request, response);
		DuoDAO.getDdao().getAllDuo(request);
		DuoDAO.getDdao().paging(1, request);
